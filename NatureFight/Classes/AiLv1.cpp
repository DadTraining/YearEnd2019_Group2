...
void AiLv1::Init()
{
	...
	physicsBodyCheck= PhysicsBody::createBox(this->m_sprite->getContentSize()*2, PhysicsMaterial(0.1f, 1.0f, 0.0f));
	...
	//physic check
	physicsBodyCheck->setDynamic(false);
	physicsBodyCheck->setRotationEnable(false);
	physicsBodyCheck->setCollisionBitmask(101);
	physicsBodyCheck->setContactTestBitmask(1);
	this->m_sprite->addComponent(physicsBodyCheck);
	physicsBodyCheck->setGravityEnable(false);
}
float timem = 0;
void AiLv1::Collision(Player* player, float deltaTime)
{

	if ((Distance(this->m_sprite->getPosition(),player->m_sprite->getPosition()))<100) {
		timem += deltaTime;
		if (timem >= 2) {
